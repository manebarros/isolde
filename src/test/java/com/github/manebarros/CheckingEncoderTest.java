...
public interface CheckingEncoderTest {
  ...
  default void commitOrderExtendsSoPlusWr() {
    ...
    Contextualized<KodkodProblem> p =
        encoder().encode(hist, (h, co) -> h.sessionOrder().union(h.binaryWr()).in(co).not());
    Solution sol = new Solver().solve(p.getContent().formula(), p.getContent().bounds());
    assertTrue(sol.unsat());
  }
        ...
        new History(
            new Session(
                Arrays.asList(
                    ...
                    new Transaction(3, Arrays.asList(readOf(0, 0))))));
    Contextualized<KodkodProblem> p =
        encoder().encode(hist, (h, co) -> KodkodUtil.strictTotalOrder(co, h.transactions()).not());
    Solution sol = new Solver().solve(p.getContent().formula(), p.getContent().bounds());
    assertTrue(sol.unsat());
  }
  ...
  default void initialTxnIsFirstInCommitOrder() {
    ...
    Contextualized<KodkodProblem> p =
        encoder()
            .encode(
                hist,
                (h, co) -> KodkodUtil.min(h.initialTransaction(), co, h.transactions()).not());
    Solution sol = new Solver().solve(p.getContent().formula(), p.getContent().bounds());
    assertTrue(sol.unsat());
  }
